...
const DocumentPreview: React.FC<Props> = ({ evaluation: initialEvaluation, settings, isEditable = true, onSettingsChange }) => {
  ...
  const handleInlineInput = (e: React.FormEvent<HTMLElement>) => {
    ...
    const preCursorText = text.slice(0, selection.focusOffset);
  ...
  const editableTitleStyles = "focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:bg-indigo-50 rounded px-1 transition-all duration-200 cursor-text min-w-[50px] inline-block";
  const editableParagraphStyles = "focus:outline-none focus:ring-2 focus:ring-slate-300 focus:bg-slate-50 rounded px-1 transition-all duration-200 cursor-text min-w-[50px] inline-block";
  ...
  return (
    <div 
      ...
      id="printable-eval"
    >
      {/* Universal Autocomplete Popup */}
      {autocomplete.show && (
        <div 
          className="fixed z-[100] bg-white border border-slate-200 rounded-xl shadow-2xl py-2 min-w-[180px] no-print animate-in fade-in zoom-in-95 duration-100 ring-4 ring-indigo-500/5"
          style={{ left: autocomplete.x, top: autocomplete.y + 5 }}
        >
          <div className="px-3 py-1.5 border-b border-slate-50 flex justify-between items-center bg-slate-50/50 mb-1">
            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Sugerencias LaTeX</span>
          </div>
          <div className="max-h-48 overflow-y-auto scrollbar-thin">
            {LATEX_COMMANDS.filter(cmd => cmd.label.toLowerCase().includes(autocomplete.filter.toLowerCase())).map((cmd, i) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-[10px] flex items-center justify-between gap-4 transition-colors ${autocomplete.index === i ? 'bg-indigo-600 text-white' : 'text-slate-700 hover:bg-indigo-50'}`}
              >
                <code className="font-bold">{cmd.label}</code>
                <span className={`text-[7px] font-serif ${autocomplete.index === i ? 'text-indigo-200' : 'text-slate-400'}`}>{cmd.icon}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {isEditable && (
        <div className="absolute top-4 right-4 no-print flex flex-col items-end gap-2">
          ...
          <p className="text-[9px] text-slate-400 font-medium bg-white/80 px-2 py-1 rounded text-right">Usa <b>\</b> para comandos matemáticos rápidos.</p>
        </div>
      )}
      ...
      {settings.showInstructions && (
        <div className="mb-8 border-2 border-dashed border-slate-300 rounded-xl p-5 bg-slate-50/30 print-avoid-break">
          ...
          <div contentEditable={isEditable} suppressContentEditableWarning onInput={handleInlineInput} onBlur={handleValidation("Lea atentamente cada pregunta antes de responder.")} className={`text-[10px] text-slate-600 leading-relaxed italic text-left ${isEditable ? editableParagraphStyles : ""}`}>
      ...
      {/* Curriculum Context (Alineación Curricular) */}
        ...
        <div className="p-4 sm:p-8 space-y-6 bg-gradient-to-b from-slate-50/50 to-white">
          <div className="relative overflow-hidden border-l-4 border-indigo-600 pl-4 sm:pl-6 bg-indigo-50/30 py-4 sm:py-5 pr-4 rounded-r-2xl shadow-sm ring-1 ring-indigo-500/10">
            ...
            <div className="text-[11px] font-bold text-slate-800 leading-relaxed italic text-left">
              <span className={`text-indigo-900 mr-2 underline underline-offset-4 decoration-indigo-200 font-black px-1 ${isEditable ? editableTitleStyles : ""}`} contentEditable={isEditable} suppressContentEditableWarning onInput={handleInlineInput} onBlur={handleValidation(evaluation.oa_code, 'oa_code')}>{evaluation.oa_code}</span>: 
              <span contentEditable={isEditable} suppressContentEditableWarning onInput={handleInlineInput} onBlur={handleValidation(evaluation.oa_description, 'oa_description')} className={`ml-1 leading-relaxed ${isEditable ? editableParagraphStyles : ""}`}>"{evaluation.oa_description}"</span>
            </div>
          </div>
                          ...
                          <div className="flex justify-around items-center gap-2">
                            {['L', 'P', 'PL'].map(label => (
                              <div key={label} className="flex flex-col items-center gap-1.5 shrink-0">
                                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg border-2 border-slate-200 bg-white shadow-inner"></div>
                                <span className="text-[7px] sm:text-[8px] font-black text-slate-400 uppercase tracking-tighter">{label}</span>
                              </div>
                            ))}
                          </div>
                        </td>
      ...
      {/* Habilidades y Actitudes Evaluadas (Despues de Alineación Curricular) */}
      <div className="mb-10 border-2 border-emerald-800 rounded-2xl overflow-hidden print-avoid-break bg-white shadow-xl ring-1 ring-emerald-900/5">
        ...
        <div className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-10">
            <div className="space-y-4">
              <h5 className="text-[10px] font-black text-emerald-700 uppercase tracking-widest flex items-center gap-2 pb-2 border-b border-emerald-50 text-left">
              ...
              <div className="flex flex-wrap gap-2 text-left">
                {evaluation.skills?.map((skill, i) => (
                  <span 
                    ...
                    className={`text-[9px] font-black px-3 py-1 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded-full uppercase shadow-sm ${isEditable ? editableTitleStyles : ""}`}
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-2.5 text-left">
                {evaluation.attitudes?.map((att, i) => (
                  <li key={i} className="text-[10px] text-slate-600 leading-tight flex gap-3 items-start">
                    <span className="font-black text-emerald-500 text-xs shrink-0 mt-0.5">●</span>
                    <span 
                      contentEditable={isEditable} 
                      suppressContentEditableWarning 
                      onInput={handleInlineInput}
                      onBlur={handleValidation(att, 'attitude', i)} 
                      className={`block w-full rounded px-2 py-1 ${isEditable ? editableParagraphStyles : ""}`}
                    >
                      {att}
                    </span>
                  </li>
        ...
        {evaluation.sections.map((section, idx) => (
          <div key={idx} className="print-avoid-break relative group">
            <div className="flex items-center gap-4 mb-4">
              ...
              <h2 contentEditable={isEditable} suppressContentEditableWarning onInput={handleInlineInput} onBlur={handleValidation(section.title, 'section_title', idx)} className={`text-lg font-black text-slate-800 uppercase tracking-tight border-b-2 border-slate-900 flex-1 text-left ${isEditable ? editableTitleStyles : ""}`}>{section.title}</h2>
              {isEditable && (
      ...
      <div className="mt-auto pt-10 print-avoid-break">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 border-t-2 border-slate-100 pt-10">
           ...
           <div className="p-8 bg-indigo-50/30 rounded-3xl border border-indigo-100 shadow-sm space-y-4">
              ...
              <div contentEditable={isEditable} suppressContentEditableWarning onInput={handleInlineInput} onBlur={handleValidation("[Sin retroalimentación definida]")} className={`min-h-[100px] focus:outline-none text-left rounded-xl p-2 leading-relaxed text-slate-700 italic border-2 border-dashed border-indigo-100/50 ${isEditable ? editableParagraphStyles : ""}`}>
